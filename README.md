...
Reasoning levels come from each log turn's `turn_context.effort`, associated by turn ID. Missing or unrecognized levels display as **Unspecified**. Token checkpoint schema 2 reads schema 1 without dropping totals: a bounded replay enriches old hourly/model buckets only when every counter and the response count exactly matches the original bucket. Missing or changed sources keep their original unspecified totals. New responses continue collecting during this replay; copied responses never increase totals. CSV exports include `reasoning_level` (app exports append the column to preserve existing field positions).
