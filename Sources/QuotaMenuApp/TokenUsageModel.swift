...
final class TokenUsageModel: ObservableObject {
        ...
        collection = Task { [weak self] in
            while !Task.isCancelled {
                ...
                do {
                    ...
                    self?.failureMessage = "Token history unavailable: \(error.localizedDescription)"
                }
    ...
    func recordQuota(_ usage: UsageSnapshot) async {
        ...
        catch { failureMessage = "Quota audit could not be saved." }
    }
        ...
        Task {
            ...
            do {
                ...
                let export = Task.detached(priority: .utility) {
                    ...
                    let header = "record_type,timestamp_utc,model,input_tokens,cached_input_tokens,cache_write_input_tokens,output_tokens,reasoning_output_tokens,total_tokens,quota_used_percent,reset_utc,account_partition,reasoning_level"
                    var rows = [header]
                    for bin in report.bins {
                        let c = bin.counts
                        let model = "\"" + bin.model.replacingOccurrences(of: "\"", with: "\"\"") + "\""
                        rows.append("tokens,\(formatter.string(from: bin.hour)),\(model),\(c.input),\(c.cachedInput),\(c.cacheWriteInput),\(c.output),\(c.reasoningOutput),\(c.total),,,,\(bin.reasoningLevel)")
                    }
                    if !quota.isEmpty { rows.append(quota.split(separator: "\n").map { String($0) + "," }.joined(separator: "\n")) }
            ...
            } catch { failureMessage = "Export failed: \(error.localizedDescription)" }
        }
    }
}
